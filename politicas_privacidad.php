...
try {
  ...
  $stmt_pol = $pdo->prepare("
        ...
        WHERE nombre_politica = 'Políticas de Privacidad del Portal Web' 
        AND activo = 1 
        LIMIT 1
    ");
  ...
  $fecha_formateada = date("d/m/Y", strtotime($politica['fecha_actualizacion']));
    ...
    die("Error de conexión al cargar las políticas de privacidad.");
}
?>
<!DOCTYPE html>
<html lang="es-ES">
<head>
   ...
   <!-- Vinculamos la hoja de estilos de privacidad corregida sin el bug de line-height -->
   <link href="estilos/politicas_privacidad.css" rel="stylesheet">
...
<body>
   ...
   <table class="table01">
      <tr>
        <td class="cabecera01">
          <!-- Reemplazamos el título estático de aclaración por la Fecha de actualización dinámica de la base de datos -->
          <h2>ÚLTIMA ACTUALIZACIÓN: <?php echo $fecha_formateada; ?></h2>
        </td>
      </tr>
      <tr>
        <td class="cuerpo01">
          <!-- Impresión del cuerpo del texto en HTML directo de la base de datos -->
