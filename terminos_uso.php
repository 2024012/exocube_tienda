...
try {
  // CORRECCIÓN ELÁSTICA: Jala de forma automática el documento que el admin encendió (activo = 1) en el panel
  $stmt_pol = $pdo->prepare("
  SELECT * FROM politicas_portal 
  WHERE nombre_politica = 'Términos y Condiciones de Uso del Portal Web' 
  AND activo = 1 
  LIMIT 1
");
  ...
  $fecha_formateada = date("d/m/Y", strtotime($politica['fecha_actualizacion']));
    ...
    die("Error de conexión al cargar las políticas de uso.");
}
?>
<!DOCTYPE html>
<html lang="es-ES">
<head>
   ...
   <!-- Vinculamos la hoja de estilos corregida sin el bug de line-height -->
   <link href="estilos/terminos_uso.css" rel="stylesheet">
